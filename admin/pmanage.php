<?php
	...
	if (isset($_POST['update'])){
		...
		if (empty($updatePrice) || empty($updateName) || empty($updateUnit) || empty($updateLength) || empty($updateMbt) || empty($updateconcurrents)){
			...
			$SQLinsert = $odb -> prepare("UPDATE `plans` SET `name` = :name, `vip` = :vip, `mbt` = :mbt, `unit` = :unit, `length` = :length, `price` = :price, `concurrents` = :concurrents, `private` = :private WHERE `ID` = :id");
			$SQLinsert -> execute(array(':name' => $updatevip, ':vip' => $updateMbt, ':mbt' => $updateMbt, ':unit' => $updateUnit, ':length' => $updateLength, ':price' => $updatePrice, ':concurrents' => $updateconcurrents, ':private' => $updateprivate, ':id' => $_POST['update']));
	...
	if (isset($_POST['addplan'])){
		...
		/* if (empty($price) || empty($name) || empty($unit) || empty($length) || empty($mbt) || empty($concurrents)|| empty($vip))
		{
			...
			$SQLinsert = $odb -> prepare("INSERT INTO `plans` VALUES(NULL, :name, :vip, :mbt, :unit, :length, :price, :concurrents, :private)");
			$SQLinsert -> execute(array(':name' => $name, ':vip' => $vip, ':mbt' => $mbt, ':unit' => $unit, ':length' => $length, ':price' => $price, ':concurrents' => $concurrents, ':private' => $private));
